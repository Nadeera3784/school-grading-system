...
Public Class frmComputeSubjectGradeMain

    Public isComputingGradeMain As Boolean
    Public isComputingGradeTeacher As Boolean
    ...
    Public Sub LoadSubjectGrade(ByVal gender As String)
        Try
            sqL = "SELECT SGID, CONCAT(Lastname, ', ', Firstname, ' ', MI) as SName,  KQuiz, KExam, KTotal, KEquivalent, KPercent, PQuiz, PExam, PTotal, PEquivalent, UPercent, UQuiz, UExam, UTotal, UEquivalent, UPercent, PPPP, PPEquivalent, PPPercent, FG, LG, TIQuiz, TIExam FROM SubjectGrade sg INNER JOIN Student s ON sg.StudentNo = s.StudentNo WHERE sg.SY = '" & cmbSY.Text & "' AND sg.SectionNO = '" & txtSection.Tag & "' AND Subject = '" & cmbSubject.Text & "' AND GradingPeriod = '" & cmbGrading.Text & "' AND Gender= '" & gender & "' ORDER BY Lastname, Firstname"
            ConnDB()
            cmd = New MySqlCommand(sqL, conn)
            dr = cmd.ExecuteReader
